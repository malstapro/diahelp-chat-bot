...
async def finish_state(state: FSMContext):
    try:
        async with state.proxy() as data:
            ...
            await state.finish()
        async with state.proxy() as data:
            data['add-sugar-last-time'] = tmp[0]
            data['chart-last-time'] = tmp[1]
...
async def statistics(m: types.Message, state: FSMContext):
    try:
        async with state.proxy() as data:
            try:
                if (timedelta(hours=5) - (
                            datetime.now(tz=pytz.timezone("Europe/Kiev")) - data['chart-last-time'])) <= timedelta(
                            minutes=0, seconds=0):
                    ...
                    await bot.send_message(m.from_user.id, messages.waite_add_sugar.format(
                        ...
                        'години' if wait_time > timedelta(hours=1) else 'годину' if wait_time == timedelta(
                            hours=1) else 'хвилин' if wait_time > timedelta(minutes=4) else "хвилини"), parse_mode=ParseMode.MARKDOWN)
            except KeyError:
                sugars = sugar[m.from_user.id]['sugars'][f'{datetime.now().year}'][f'{datetime.now().month}'][f'{datetime.now().day}']
                time_list = []
                index_list = []
